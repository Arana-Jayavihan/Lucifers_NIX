class BrightnessService extends Service {
  ...
  set screen_value(percent) {
    ...
    if (percent == this.#screenValue) return;

    this.#screenValue = percent;
    this.#date = Number(new Date());
    // the file monitor is slow
  ...
  #onChange() {
    if (Number(new Date()) - this.#date > 20) {
      Utils.execAsync("brightnessctl get").then((val) => {
        const new_value = Number(val) / this.#max;
        if (new_value === this.#screenValue) return;

        this.#screenValue = new_value;
  ...
  connect(event = "screen-changed", callback: (...any: any[]) => any) {
