...
pkgs.writeShellScriptBin \"list-hypr-bindings\" \'\' 
  ...
  --column=Description: \\
  --timeout=60 \\
  --timeout-indicator=right \\ " >> "${flakeDir}"/config/scripts/test.nix;

for bind in "$\{newBindings[@]\}"; do
  echo "  $bind" >> "${flakeDir}"/config/scripts/test.nix
done;

echo "  \"ALT + TAB\" \"Cycle Window Focus + Bring To Front\" \\" >> "${flakeDir}"/config/scripts/test.nix;
echo "\"\"" >> "${flakeDir}"/config/scripts/test.nix;
echo "\'\'" >> "${flakeDir}"/config/scripts/test.nix;
''
